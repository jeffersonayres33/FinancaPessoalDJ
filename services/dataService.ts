...
const sanitizeTransactionPayload = (despesa: Despesa, dataContextId?: string) => {
    ...
    if (despesa.installments) {
        installmentsPayload = {
            current: despesa.installments.current,
            total: despesa.installments.total,
            isAdvancePayment: despesa.isAdvancePayment === true || despesa.installments.isAdvancePayment === true
        };
    }
    ...
    return {
        ...
        ...(despesa.createdAt ? { created_at: despesa.createdAt } : {}),
        ...(despesa.updatedAt ? { updated_at: despesa.updatedAt } : {})
    };
};
...
export const dataService = {
  ...
  deleteCategoriesBulk: async (ids: string[]): Promise<void> => {
  ...
  fetchTransactions: async (dataContextId: string): Promise<Despesa[]> => {
    ...
    const fetchPromise = (async () => {
        ...
        return (data || []).map((t: any) => {
          const isAdvance = t.installments?.isAdvancePayment === true || t.is_advance_payment === true || t.isAdvancePayment === true;
          return {
            ...t,
  ...
  addTransactions: async (despesas: Despesa[], dataContextId: string): Promise<Despesa[]> => {
    if (despesas.length === 0) return [];
    
    const payload = despesas.map(d => ({
        ...sanitizeTransactionPayload(d, dataContextId),
        id: d.id
    }));
