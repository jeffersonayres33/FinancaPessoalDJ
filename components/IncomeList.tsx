                    ...
                    <tr key={t.id} className={`hover:bg-gray-50 transition-colors cursor-pointer ${selectedIds.includes(t.id) ? 'bg-purple-50/50' : ''}`} onClick={() => {
                      ...
                      <td className="p-3">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {t.isFixed && (
                            <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 flex items-center gap-1 whitespace-nowrap" title="Receita Fixa (Recorrente)">
                              <Repeat size={10} /> Fixa
                            </span>
                          )}
                          {t.installments && t.installments.total > 1 && (
