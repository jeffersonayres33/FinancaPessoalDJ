           ...
           <div className="flex gap-2 w-full sm:w-auto">
              <div className="flex bg-gray-100 rounded-md p-1 mr-2">
                <button 
                  ...
                  className={`p-1.5 rounded-sm ${viewMode === 'grid' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500 hover:text-gray-700'}`} 
                ...
                <button 
                  ...
                  className={`p-1.5 rounded-sm ${viewMode === 'list' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500 hover:text-gray-700'}`} 
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center mb-4">
           ...
           <div className="flex gap-2 w-full md:w-auto">
             ...
             <div className="flex bg-gray-100 rounded-full p-1">
                ...
                <button onClick={() => { setSortBy('createdAt'); setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc'); }} className={`p-2 rounded-full ${sortBy === 'createdAt' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500'}`} title="Ordenar por Data de Criação"><Clock size={16} /></button>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4 pt-4 border-t border-gray-100 text-sm gap-4">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              {filteredExpenses.length > 0 && (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedIds(filteredExpenses.map(t => t.id));
                      } else {
                        setSelectedIds([]);
                      }
                    }}
              ...
              {selectedIds.length > 0 && onBulkDeleteDespesas && (
                <button
                  onClick={() => {
                    onBulkDeleteDespesas(selectedIds);
                    setSelectedIds([]);
                  }}
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <div className="flex gap-2 mt-1 flex-wrap">
                         ...
                         {t.isFixed && (
                           <span className={`text-xs font-medium px-2 py-0.5 rounded flex items-center gap-1 ${t.installments?.current === 0 ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'}`} title={t.installments?.current === 0 ? "Despesa Fixa (Inativa)" : "Despesa Fixa (Recorrente)"}>
                    ...
                    <tr key={t.id} className={`hover:bg-gray-50 transition-colors cursor-pointer ${selectedIds.includes(t.id) ? 'bg-purple-50/50' : ''}`} onClick={() => {
                      ...
                      <td className="p-3">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {t.isFixed && (
                            <span className={`text-xs font-medium px-1.5 py-0.5 rounded flex items-center gap-1 whitespace-nowrap ${t.installments?.current === 0 ? 'bg-red-100 text-red-700' : 'bg-indigo-100 text-indigo-700'}`} title={t.installments?.current === 0 ? "Despesa Fixa (Inativa)" : "Despesa Fixa (Recorrente)"}>
                              <Repeat size={10} /> {t.installments?.current === 0 ? 'Fixa (Inativa)' : 'Fixa'}
                            </span>
                          )}
                          {t.installments && t.installments.total > 1 && (
