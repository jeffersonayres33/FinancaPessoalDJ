...
export const DespesaList: React.FC<DespesaListProps> = ({ despesas, onDeleteDespesa, categories }) => {
  ...
  }, [despesas, searchTerm, categoryFilter, dateFilter]);
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredDespesas.map((despesa) => (
              <tr key={despesa.id} className="hover:bg-gray-50 transition-colors">
                 <td className="px-6 py-4 whitespace-nowrap">
                  {despesa.status === 'paid' ? (
                    <span className="flex items-center text-green-600 text-xs font-medium bg-green-100 px-2 py-1 rounded-full w-fit">
                      ...
                      {despesa.type === 'income' ? 'Recebido' : 'Pago'}
                    </span>
                  ) : (
                ...
                <td className="px-6 py-4 text-sm font-medium text-gray-900">{despesa.title}</td>
                <td className={`px-6 py-4 text-sm font-medium ${despesa.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
                  {despesa.type === 'expense' && '- '}
                  {formatCurrency(despesa.amount)}
                </td>
                <td className="px-6 py-4 text-sm text-gray-500">{despesa.category}</td>
                <td className="px-6 py-4 text-sm text-gray-500">{formatDate(despesa.date)}</td>
